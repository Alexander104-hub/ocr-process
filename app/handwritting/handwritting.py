...
class HandwritingRecognitionModel:
    def __init__(self, model_path='model/my_model.h5'):
        ...
        class CERMetric(tf.keras.metrics.Metric):
            ...
            def update_state(self, y_true, y_pred, sample_weight=None):
                ...
                decode = tf.keras.backend.ctc_label_dense_to_sparse(decode[0], tf.cast(input_length, 'int32'))
                y_true_sparse = tf.keras.backend.ctc_label_dense_to_sparse(y_true, tf.cast(input_length, 'int32'))
        ...
        self.model = load_model('model/my_model.h5', custom_objects={
            'LeakyReLU': LeakyReLU,
            'CTCLoss': CTCLoss,
            'CERMetric': CERMetric
        })
    ...
    def preprocess_image(self, img):
        ...
        resized_img = cv2.adaptiveThreshold(resized_img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)
    ...
    def decode_text(self, prediction):
        ...
        decoded = get_value(ctc_decode(prediction, input_length=input_length, greedy=True)[0][0])
        
        # Convert indices to characters
        texts = []
        for values in decoded:
            text = ''.join([self.alphabet[i] for i in values if i >= 0 and i < len(self.alphabet)])
            texts.append(text)
    ...
    def extract_polygons(self, image_path, gt_file, recognize_text=False, model=None):
        ...
        for i, points in enumerate(polygons):
            ...
            if recognize_text and model:
                ...
                for row in range(h):
                    for col in range(w):
                        if mask[row, col] == 255 and row < cropped_gray.shape[0] and col < cropped_gray.shape[1]:
