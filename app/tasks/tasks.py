...
class ProcessImage(luigi.Task):
    ...
    def requires(self):
        return GenerateGroundTruth(image_path=self.image_path, output_dir=self.output_dir)
    ...
    def run(self):
        ...
        results = processor.extract_polygons(self.image_path, gt_path, recognize_text=True, model=handwritting_model)
        ...
        for i, result in enumerate(results):
            serializable_result = {
                ...
                'polygon': result.get('polygon', []).tolist() if hasattr(result.get('polygon', []), 'tolist') else result.get('polygon', []),
...
class OCRPipeline(luigi.WrapperTask):
    ...
    def requires(self):
        return ProcessImage(image_path=self.image_path, model_path=self.model_path, output_dir=self.output_dir, id=self.id)
