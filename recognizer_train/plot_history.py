...
plt.plot(
    epochs, history["val_CER_metric"], "purple", linewidth=2, label="Validation CER"
)
...
val_train_loss_ratio = [
    v / t if t != 0 else 0 for v, t in zip(history["val_loss"], history["loss"])
]
val_train_cer_ratio = [
    v / t if t != 0 else 0
    for v, t in zip(history["val_CER_metric"], history["CER_metric"])
]
...
loss_improvements = [
    history["loss"][i] - history["loss"][i + 1] for i in range(len(history["loss"]) - 1)
]
cer_improvements = [
    history["CER_metric"][i] - history["CER_metric"][i + 1]
    for i in range(len(history["CER_metric"]) - 1)
]
