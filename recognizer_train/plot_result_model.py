...
# Load the model
model = keras.models.load_model('model/my_model.h5', custom_objects={
            'LeakyReLU': LeakyReLU,
            'CTCLoss': CTCLoss,
            'CERMetric': CERMetric
        })
...
for i, true_char in enumerate(all_chars):
    for j, pred_char in enumerate(all_chars):
        if i != j and conf_matrix[i, j] > 0:
            confusion_counts.append((true_char, pred_char, conf_matrix[i, j], 
...
plt.axvline(x=np.mean(cers), color='red', linestyle='--', label=f'Mean CER: {np.mean(cers):.3f}')
plt.axvline(x=np.median(cers), color='green', linestyle='--', label=f'Median CER: {np.median(cers):.3f}')
...
char_error_df = pd.DataFrame({'Character': list(char_errors.keys()), 
...
# Create a heatmap
sns.heatmap(conf_subset, annot=True, fmt='.2f', cmap='Blues',
...
conf_df = pd.DataFrame(top_confusions, columns=['True', 'Predicted', 'Count', 'Percentage'])
...
p = np.poly1d(z)
plt.plot(sorted(text_lengths), p(sorted(text_lengths)), "r--", 
...
sample_indices = np.random.choice(range(len(true_texts)), min(5, len(true_texts)), replace=False)

for i, idx in enumerate(sample_indices):
    plt.subplot(5, 1, i+1)
    plt.imshow(np.reshape(test_X[idx], (32, 732)), cmap='gray')
    plt.axis('off')
    plt.title(f"True: '{true_texts[idx]}' | Predicted: '{predicted_texts[idx]}' | CER: {cers[idx]:.3f}")
plt.tight_layout()
plt.show()
...
cer_length_df = pd.DataFrame({
    'CER': cers,
    'Length Category': length_categories
})
...
char_freq_percentage = {char: count/total_chars*100 for char, count in char_freq_counts.items()}

# Create dataframe for visualization
char_analysis_df = pd.DataFrame({
    'Character': list(char_errors.keys()),
    'Error Rate': list(char_errors.values()),
    'Frequency (%)': [char_freq_percentage.get(char, 0) for char in char_errors.keys()]
})

# Plot scatter with size proportional to frequency
plt.figure(figsize=(12, 8))
plt.scatter(char_analysis_df['Frequency (%)'], char_analysis_df['Error Rate'], 
           s=100, alpha=0.7, edgecolors='w', linewidth=0.5)

# Add character labels to points
for i, row in char_analysis_df.iterrows():
    plt.annotate(row['Character'], 
...
if len(available_chars) > 0:
    ...
    cyrillic_indices = [char_to_idx[char] for char in available_chars if char in char_to_idx]
    
    # Create a subset confusion matrix
    cyrillic_conf = conf_matrix_norm[np.ix_(cyrillic_indices, cyrillic_indices)]
    
    plt.figure(figsize=(10, 8))
    sns.heatmap(cyrillic_conf, annot=True, fmt='.2f', cmap='Blues',
...
for i, (true_char, pred_char, count, percentage) in enumerate(confusion_counts[:8]):
    print(f"\"{true_char}\" and \"{pred_char}\" ({percentage:.1f}% of all errors for \"{true_char}\")")
