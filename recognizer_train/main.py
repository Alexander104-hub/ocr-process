...
class AugmentationDataGenerator(tf.keras.utils.Sequence):
    ...
    def apply_weak_augmentation(self, img):
        ...
        aug_type = np.random.choice(
            ["minimal_rotation", "noise", "brightness", "elastic"],
            p=[0.3, 0.3, 0.2, 0.2],
        )
...
history = model.fit(
    ...
    callbacks=[
        EarlyStopping(
            patience=15, restore_best_weights=True, monitor="val_CER_metric", mode="min"
        ),
        ReduceLROnPlateau(
            factor=0.5, min_lr=1e-5, patience=4, monitor="val_CER_metric", mode="min"
        ),
        model_checkpoint,
    ],
    verbose=1,
