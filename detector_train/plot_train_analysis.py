...
def visualize_loss_curves(data):
    ...
    train_improvement = [0] + [
        (train_losses[i - 1] - train_losses[i]) for i in range(1, len(train_losses))
    ]
    val_improvement = [0] + [
        (val_losses[i - 1] - val_losses[i]) for i in range(1, len(val_losses))
    ]
...
def visualize_time_metrics(data):
    ...
    plt.subplot(2, 1, 2)
    plt.bar(
        epochs, data_loading_times, width=0.4, align="edge", label="Data Loading Time"
    )
    plt.bar(
        [e + 0.4 for e in epochs],
        processing_times,
        width=0.4,
        align="edge",
...
def visualize_early_stopping(data):
    ...
    plt.plot(
        ...
        smoothed_val_losses,
        "r--",
        label=f"Smoothed Val Loss (window={window_size})",
    )
    ...
    plt.axvline(
        ...
        color="g",
        linestyle="--",
        label=f"Min Val Loss at Epoch {min_loss_epoch}",
    )
...
def analyze_training_progression(data):
    ...
    print(
        f"Loss reduction: {train_losses[0] - train_losses[-1]:.4f} ({(1 - train_losses[-1]/train_losses[0])*100:.2f}%)"
    )

    print(f"\nInitial validation loss: {val_losses[0]:.4f}")
    print(f"Final validation loss: {val_losses[-1]:.4f}")
    print(
        f"Loss reduction: {val_losses[0] - val_losses[-1]:.4f} ({(1 - val_losses[-1]/val_losses[0])*100:.2f}%)"
    )
    ...
    recent_val_trend = [
        val_losses[-i - 1] - val_losses[-i] for i in range(val_window - 1)
    ]
    ...
    print(
        f"Estimated remaining time: {remaining_time/60:.2f} minutes ({remaining_time/3600:.2f} hours)"
    )
