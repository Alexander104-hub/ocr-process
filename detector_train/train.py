    ...
    for i, sample in enumerate(train_loader):
        ...
        if i % 10 == 0:
            ...
            if i == 0 and epoch % 5 == 0:
                ...
                writer.add_image(
                    "Train/PredRegion", pred_heatmap.transpose(2, 0, 1), epoch
                )
                writer.add_image(
                    "Train/TargetRegion", target_heatmap.transpose(2, 0, 1), epoch
                )
...
def validate(
    net, val_loader, criterion, device, epoch, writer, neg_rto=3.0, n_min_neg=10000
):
    net.eval()
    batch_time = AverageMeter()
    losses = AverageMeter()
    ...
    if epoch % 5 == 0 and len(val_loader) > 0:
        with torch.no_grad():
            ...
            writer.add_image(
                "Val/TargetRegion", target_heatmap.transpose(2, 0, 1), epoch
            )
...
def main():
    ...
    parser.add_argument(
        "--val_split", default=0.15, type=float, help="validation split ratio"
    )
    parser.add_argument(
        "--pretrained_model",
        default="model/craft_mlt_25k.pth",
        type=str,
    ...
    parser.add_argument(
        "--num_epochs", default=400, type=int, help="number of training epochs"
    )
    parser.add_argument(
        "--cuda", default=True, type=str2bool, help="Use cuda for training"
    )
    parser.add_argument(
        "--checkpoint_path",
        default="model/save",
    ...
    parser.add_argument(
        "--neg_rto", default=3.0, type=float, help="negative ratio for loss calculation"
    )
    parser.add_argument(
        "--n_min_neg", default=15000, type=int, help="minimum number of negative pixels"
    )
    ...
    if args.pretrained_model:
        ...
        net.load_state_dict(
            copyStateDict(torch.load(args.pretrained_model, map_location=device))
        )
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        save_checkpoint(
            ...
            filename=os.path.join(
                args.checkpoint_path, f"checkpoint_epoch_{epoch}.pth"
            ),
        )

        torch.save(
            net.state_dict(),
            os.path.join(args.checkpoint_path, "craft_finetune_last.pth"),
        )

        if is_best:
            torch.save(
                net.state_dict(),
                os.path.join(args.checkpoint_path, "craft_finetune_best.pth"),
            )
